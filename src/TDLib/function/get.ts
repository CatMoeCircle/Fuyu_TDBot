 ...
 */
export async function getUserFullInfo(client: Client, user_id: number) {
  try {
    ...
    logger.debug(error, `getUserFullInfo param ${user_id}`);
    throw new Error(
      `获取用户详细信息失败 "${user_id}": ${error instanceof Error ? error.message : String(error)}`, { cause: error }
    );
  }
}
 ...
 */
export async function getUser(client: Client, user_id: number) {
  try {
    const user = await client.invoke({
      _: "getUser",
      user_id: user_id,
    ...
    logger.debug(error, `getUser param${user_id}`);
    throw new Error(
 ...
 */
export async function getChat(client: Client, chat_id: number) {
  try {
    ...
    logger.debug(error, `getChat param ${chat_id}`);
    throw new Error(
 ...
 */
export async function getSupergroup(client: Client, supergroup_id: number) {
  try {
    ...
    logger.debug(error, `getSupergroup param ${supergroup_id}`);
    throw new Error(
...
export async function getSupergroupFullInfo(
  ...
  try {
    ...
    logger.debug(
      error,
      `getSupergroupFullInfo param ${supergroup_id}`
    );
    throw new Error(
...
export async function getMessage(
  ...
  try {
    ...
    logger.debug(error, `getMessage param ${chat_id}, ${message_id}`);
    throw new Error(
 ...
 * @returns 包含基础群组信息的对象
 */
export async function getBasicGroup(client: Client, basic_group_id: number) {
  try {
    ...
    logger.debug(error, `getBasicGroup param ${basic_group_id}`);
    throw new Error(
...
export async function getMessageLink(
  ...
  try {
    ...
    logger.debug(
      error,
      `getMessageLink param ${chat_id}, ${message_id}`
    );
    throw new Error(
...
export async function getStickerSet(client: Client, set_id: string) {
  try {
    ...
    logger.debug(error, `getStickerSet param ${set_id}`);
    throw new Error(
 ...
 */
export async function getMessageLinkInfo(client: Client, url: string) {
  try {
    ...
    logger.debug(error, `getMessageLinkInfo param ${url}`);
    throw new Error(
 ...
 * 获取当前用户的基本信息
 * @param client - TDLib 客户端实例
 * @returns 包含当前用户信息的对象，如果发生错误则返回undefined
 * @description
 * 此函数用于获取当前登录用户的基本信息，包括用户名、ID等。
 */
export async function getMe(client: Client) {
  try {
    ...
    logger.debug(error, `getMe`);
    throw new Error(
      `getMe失败: ${error instanceof Error ? error.message : String(error)}`, { cause: error }
    );
  }
}
...
export async function getChatByUsername(client: Client, username: string) {
  try {
    ...
    logger.debug(error, `getChatByUsername param ${username}`);
    throw new Error(
      `获取聊天失败 "${username}": ${error instanceof Error ? error.message : String(error)}`, { cause: error }
    );
  }
}